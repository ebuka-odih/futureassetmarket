                ...
                <div class="mainchart px-3 px-md-4 py-3 py-lg-4 ">
                    ...
                    <div class="row mt-5">
                                ...
                                <script type="text/javascript"
                                    ...
                                    :
                                        "500",
                                            "symbol"
                                    :
                                        "{{ in_array($holding->stock->symbol, $nyseStocks) ? 'NYSE:' : 'NASDAQ:' }}{{ $holding->stock->symbol ?? '' }}",
                                            "interval"
                                    :
                                        "D",
                                            "timezone"
                        ...
                        <div class="col-lg-4">
                            ...
                            @endif
                            <form action="{{ route('user.storeSell') }}" method="POST">
                                @csrf
{{--                                <input type="hidden" name="stock_id" value="{{ $holding->id }}">--}}
                               <input type="hidden" name="stock_id" value="{{ $holding->stock_id }}">
                                <div class="mb-3">
                                    <div class="stock-price-display">
                                        <span class="fw-bold"> {{ $holding->stock->symbol ?? '' }} Price: </span>
                                        <span class="text-success">${{ $holding->stock->price ?? ''}}</span>
                                    </div>
                                    <div class="stock-price-display">
                                        <span class="fw-bold"> {{ $holding->stock->symbol ?? '' }} Holdings: </span>
                                        <span class="text-warning">{{ $holding->total_shares ?? ''}}</span>
                                    </div>


                                    <label for="amount" class="form-label">Amount of shares to sell</label>
                                    <input type="number" name="shares" placeholder="Shares" class="form-control" min="0" max="{{ $holding->total_shares }}" step="0.0001" required>
                                </div>
                                ...
                                <button type="submit" class="btn btn-danger w-100 mt-3">Sell</button>
                            </form>
                        </div>
                    </div>
